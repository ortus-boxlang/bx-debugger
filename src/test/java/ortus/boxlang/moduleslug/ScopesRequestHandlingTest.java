...
public class ScopesRequestHandlingTest {
	...
	public void testScopesRequestReturnsServerScope() throws Exception {
		...
		try ( SocketChannel clientSocket = SocketChannel.open() ) {
			...
			initArgs.setClientName( "BoxLang Scopes Test" );
			server.initialize( initArgs ).get( 5, TimeUnit.SECONDS );

			// Create scopes request arguments
			ScopesArguments scopesArgs = new ScopesArguments();
			scopesArgs.setFrameId( 0 ); // Mock frame ID

			// Request scopes
			ScopesResponse scopesResponse = server.scopes( scopesArgs ).get( 5, TimeUnit.SECONDS );

			// Verify scopes response
			assertNotNull( scopesResponse, "Scopes response should not be null" );
			assertNotNull( scopesResponse.getScopes(), "Scopes array should not be null" );
			assertTrue( scopesResponse.getScopes().length > 0, "Should return at least one scope" );

			// Look for server scope specifically
			boolean foundServerScope = false;
			for ( Scope scope : scopesResponse.getScopes() ) {
				if ( "server".equalsIgnoreCase( scope.getName() ) ) {
					foundServerScope = true;
					assertNotNull( scope.getVariablesReference(), "Server scope should have variables reference" );
					assertTrue( scope.getVariablesReference() > 0, "Variables reference should be positive" );
					break;
				}
			}

			assertTrue( foundServerScope, "Should find server scope in response" );
		}
	}
